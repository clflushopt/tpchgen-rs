...
mod tests {
    ...
    /// Parse one CSV line into unquoted field values
    ...
    fn row_table(row: &GeneratedRow) -> Table {
        match row {
    ...
    /// Generate a few rows from every table's generator
    fn sample_rows() -> Vec<GeneratedRow> {
        let session = Session::default();
        let mut generators: Vec<Box<dyn RowGenerator>> = vec![
        ...
        for generator in &mut generators {
            for row_number in 1..=5 {
                let result = generator
                    ...
                    .unwrap();
                rows.extend(result.get_rows().iter().cloned());
                if result.should_end_row() {
                    generator.consume_remaining_seeds_for_row();
                }
            }
    ...
    /// The CSV line of every row must parse back (honoring quoting) to
    /// exactly the row's `get_values()`, and the column count must match the
    /// table's CSV header.
    #[test]
    fn test_csv_lines_match_get_values() {
        for row in sample_rows() {
            let table = row_table(&row);
            let values = row.get_values();

            let header = csv_header(table, ',').unwrap();
            assert_eq!(
                header.split(',').count(),
                values.len(),
                "header column count mismatch for {table:?}"
            );

            let line = GeneratedRowCsv::new(&row).to_string();
            assert_eq!(
                parse_csv_line(&line, ','),
                values,
                "CSV values mismatch for {table:?}: {line}"
            );

            // Same round trip with a custom delimiter
            let line = GeneratedRowCsv::with_delimiter(&row, '|').to_string();
            assert_eq!(
                parse_csv_line(&line, '|'),
                values,
                "CSV values mismatch for {table:?} with | delimiter: {line}"
            );
        }
    }

    /// CSV lines never contain unquoted delimiters, quotes inside quoted
    /// fields, or newlines.
    #[test]
    fn test_csv_lines_are_well_formed() {
        for row in sample_rows() {
            let line = GeneratedRowCsv::new(&row).to_string();
            assert!(!line.contains('\n'), "newline in CSV line: {line}");
            // An even number of quotes, and every field is fully quoted or
            // quote-free (parse_csv_line would misalign otherwise, which
            // test_csv_lines_match_get_values catches).
            assert_eq!(
    ...
    #[test]
    fn test_headers_with_delimiter() {
        assert_eq!(
            ReasonCsv::header_with_delimiter('|'),
            "r_reason_sk|r_reason_id|r_reason_description"
        );
        assert_eq!(ReasonCsv::header_with_delimiter(','), ReasonCsv::header());
        // Column names containing the delimiter are quoted
        assert_eq!(
            ReasonCsv::header_with_delimiter('_'),
            "\"r_reason_sk\"_\"r_reason_id\"_\"r_reason_description\""
        );
        assert_eq!(
            csv_header(Table::Reason, ',').as_deref(),
            Some(ReasonCsv::header())
        );
        // Source tables have no CSV output
        assert_eq!(csv_header(Table::SCatalog, ','), None);
    }
}
