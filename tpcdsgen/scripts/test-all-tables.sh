...
# byte-for-byte (MD5 + diff) comparing Rust output against reference
# fixtures. This is the main entry point used by CI.
#
# Two reference implementations are supported, selected by --compat:
#   - trino (default): Java / Trino fixtures in tests/fixtures/scale-N-java/
#                      (generated by ./scripts/generate-fixtures.sh)
#   - c              : C dsdgen fixtures in tests/fixtures/scale-N-c/
#                      (downloaded by ./scripts/bootstrap-c.sh)
#
# What it does:
#   1. Iterates the list of 24 TPC-DS tables (dbgen_version always
#      excluded because it contains a generation timestamp).
#   2. For --compat c, additionally skips `customer` until alamb/tpcds-data
#      is regenerated without the iconv ISO-8859-14 -> UTF-8 step that
#      double-encodes non-ASCII country names. See the skip list at the
#      top of this script for the current exclusions.
#   3. Builds the Rust generator in release mode.
#   4. Delegates each per-table comparison to ./scripts/compare-table.sh.
#   5. Prints a pass/fail summary; exit non-zero if any table differs.
#
# Usage:
#   ./scripts/test-all-tables.sh                # All tables vs. Java (default)
#   ./scripts/test-all-tables.sh --compat c     # All tables vs. C dsdgen
#   ./scripts/test-all-tables.sh --scale 10     # Scale 10 vs. Java
#   ./scripts/test-all-tables.sh --quiet        # Summary-only output
