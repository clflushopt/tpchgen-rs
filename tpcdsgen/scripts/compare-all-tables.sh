...
expected hash in tests/fixtures/scale-N-{trino,c}/MD5SUMS (which ships
...
already exist locally; populate them with ./scripts/generate-fixtures.sh
(Trino) or ./scripts/generate-fixtures.sh --compat c (C dsdgen).

Two reference implementations are supported, selected by --compat:
    --compat trino  (default)  Trino TPC-DS Java reference
                               (MD5SUMS in tests/fixtures/scale-N-trino/;
                                fixtures generated by
                                ./scripts/generate-fixtures.sh, --full only)
    --compat c                 C dsdgen reference
                               (MD5SUMS in tests/fixtures/scale-N-c/;
