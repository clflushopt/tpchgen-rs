...
impl Iterator for StoreReturnsArrow {
    ...
    fn next(&mut self) -> Option<Result<RecordBatch, ArrowError>> {
        loop {
            ...
            let mut sr_returned_date: Vec<Option<i64>> = Vec::new();
            let mut sr_returned_time: Vec<Option<i64>> = Vec::new();
            let mut sr_item: Vec<Option<i64>> = Vec::new();
            let mut sr_customer: Vec<Option<i64>> = Vec::new();
            let mut sr_cdemo: Vec<Option<i64>> = Vec::new();
            let mut sr_hdemo: Vec<Option<i64>> = Vec::new();
            let mut sr_addr: Vec<Option<i64>> = Vec::new();
            let mut sr_store: Vec<Option<i64>> = Vec::new();
            let mut sr_reason: Vec<Option<i64>> = Vec::new();
            let mut sr_ticket: Vec<Option<i64>> = Vec::new();
            let mut sr_quantity: Vec<Option<i32>> = Vec::new();
            let mut sr_return_amt: Vec<Option<i128>> = Vec::new();
            let mut sr_return_tax: Vec<Option<i128>> = Vec::new();
            let mut sr_return_amt_inc_tax: Vec<Option<i128>> = Vec::new();
            let mut sr_fee: Vec<Option<i128>> = Vec::new();
            let mut sr_return_ship_cost: Vec<Option<i128>> = Vec::new();
            let mut sr_refunded_cash: Vec<Option<i128>> = Vec::new();
            let mut sr_reversed_charge: Vec<Option<i128>> = Vec::new();
            let mut sr_store_credit: Vec<Option<i128>> = Vec::new();
            let mut sr_net_loss: Vec<Option<i128>> = Vec::new();

            while self.current_row <= self.row_count && sr_returned_date.len() < self.batch_size {
                ...
                for g in result.get_rows() {
                    if let GeneratedRow::StoreReturns(r) = g {
                        ...
                        sr_return_amt_inc_tax.push(opt(nbm, 13, decimal_to_i128(p.get_net_paid_including_tax())));
