...
impl Iterator for WebSiteArrow {
    ...
    fn next(&mut self) -> Option<Result<RecordBatch, ArrowError>> {
        ...
        let tax_arr = Decimal128Array::from(web_tax_pct).with_precision_and_scale(38, 2).unwrap();

        Some(RecordBatch::try_new(Arc::clone(self.schema()), vec![
            Arc::new(Int64Array::from(web_sk)),
            Arc::new(string_view_array_from_opt_iter(web_id.iter().map(|s| s.as_deref()))),
            Arc::new(Date32Array::from(web_rec_start)),
            Arc::new(Date32Array::from(web_rec_end)),
            Arc::new(string_view_array_from_opt_iter(web_name.iter().map(|s| s.as_deref()))),
            Arc::new(Int64Array::from(web_open_date)),
            Arc::new(Int64Array::from(web_close_date)),
            Arc::new(string_view_array_from_opt_iter(web_class.iter().map(|s| s.as_deref()))),
            Arc::new(string_view_array_from_opt_iter(web_manager.iter().map(|s| s.as_deref()))),
            Arc::new(Int32Array::from(web_market_id)),
            Arc::new(string_view_array_from_opt_iter(web_market_class.iter().map(|s| s.as_deref()))),
            Arc::new(string_view_array_from_opt_iter(web_market_desc.iter().map(|s| s.as_deref()))),
            Arc::new(string_view_array_from_opt_iter(web_market_manager.iter().map(|s| s.as_deref()))),
            Arc::new(Int32Array::from(web_company_id)),
            Arc::new(string_view_array_from_opt_iter(web_company_name.iter().map(|s| s.as_deref()))),
