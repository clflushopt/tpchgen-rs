...
// TODO potentially put this in the main tpchgen crate
// behind a flag
...
/// Converts a set of sources into a parquet file
///
/// sources: Iterator of RecordBatchIterators: each RecordBatchIterator is encoded as its own row group
///
/// Returns the number of rows written
///
pub async fn generate_parquet<I>(writer: BufWriter<File>, iter_iter: I) -> Result<(), io::Error>
where
    I: Iterator<Item: RecordBatchIterator> + 'static,
{
    ...
    let writer_properties = WriterProperties::builder()
        ...
        // no compression:
        // andrewlamb@Andrews-MacBook-Pro-2:~/Software/tpchgen-rs$ du -s -h /tmp/tpchdbgen-rs/lineitem.parquet
        // 359M	/tmp/tpchdbgen-rs/lineitem.parquet
        // zstd compression:
        // andrewlamb@Andrews-MacBook-Pro-2:~/Software/tpchgen-rs$ du -s -h /tmp/tpchdbgen-rs/lineitem.parquet
        // 176M	/tmp/tpchdbgen-rs/lineitem.parquet
    ...
    let mut row_group_stream = futures::stream::iter(iter_iter)
        ...
        .buffered(num_tasks); // generate row groups in parallel
    ...
    ) = tokio::sync::mpsc::channel(num_tasks);
...
/// potentially encode multiple columns with different threads  by following the
/// example on the [`ArrowColumnWriter`].
///
